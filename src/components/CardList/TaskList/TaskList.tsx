...
const TaskPage = () => {
  ...
  const [menuTask, setMenuTask] = useState<TaskI>()
    ...
    <div className={'task-page'}>
      {loading ?
        <div className={'task-page__container'}>
          <div className={`task-list ${menuTask?.id ? 'task-list_active' : 'task-list_none'}`}>
            <div className={'task-list__name'}>
              <h3>{card_name}</h3>
            </div>
            <div className={'task-list__container'}>
              {taskList?.map(task => <Task key={task.id} {...task} menuTask={menuTask} setTaskMenu={setMenuTask}/>)}
            </div>
          ...
          {menuTask?.id ? <TaskMenu {...menuTask}/> : null}
        </div>
        : null
      }
    </div>
