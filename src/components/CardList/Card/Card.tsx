    ...
    <div className={`card`}>
      <div onClick={() => {
        ...
        <Link className={'card__link'} to={`/dashboard/card/${id}/${name}`}>
          ...
          <div className={'card__link__data'}>
        ...
        <svg className={'card__svg'} onClick={() => {
          ...
          <circle cx="5" cy="26" r="2" stroke="#000000"/>
        </svg>
      </div>
      {menu && active === id?
        <div className={'card__menu'}>Delete <svg
          className={'card__menu__delete-svg'}
          onClick={() => {
            CardController.delete(id).then()
            setMenu(false)
          }
          }
